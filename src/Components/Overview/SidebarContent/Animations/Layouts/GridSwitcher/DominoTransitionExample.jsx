            ...
            <div className={isGrid ? "grid grid-cols-1 md:grid-cols-3 gap-6" : "space-y-6"}>
                {items.map((item, i) => (
                    <motion.div
                        ...
                        className={`rounded-md bg-gray-100 overflow-hidden transform-gpu`}
                        style={{ transformOrigin: isGrid ? "left center" : "right center" }}
                    >
                        <motion.div
                            layout
                            className={`${isGrid ? 'p-5' : 'p-4 flex items-center'}`}
                            transition={{ type: "spring", stiffness: 300, damping: 25, delay: isGrid ? (i * 0.15) + 0.4 : ((items.length - i - 1) * 0.15) + 0.4 }}
                        >
                            <motion.img
                                layout
                                src={item.image}
